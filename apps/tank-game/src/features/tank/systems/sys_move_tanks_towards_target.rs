...
pub fn sys_move_tanks_towards_target(
    ...
    mut tank_query: Query<
        ...
        (With<Tank>, Without<TankGun>, Without<Building>),
    >,
    mut q_buildings: Query<
        (&mut Transform, &mut Building),
        (With<Building>, Without<TankGun>, Without<Tank>),
    >,
    mut gun_query: Query<
        (&mut Transform, &TankGun),
    ...
    let building_id_and_positions: HashMap<UnitId, Vec2> = q_buildings
        .iter()
        .map(|(tr, building)| (building.id().clone(), tr.translation.xy()))
        .collect();

    let mut all_id_and_positions: HashMap<UnitId, Vec2> = HashMap::new();
    all_id_and_positions.extend(tank_id_and_positions);
    all_id_and_positions.extend(building_id_and_positions);
    ...
    for (mut transform, mut tank) in tank_query.iter_mut().filter(|(_, tank)| tank.is_moving()) {
        ...
        if tank.get_stop_when_target_in_range() {
            ...
            if let Some(target) = tank
                .get_target()
                .and_then(|target_id| all_id_and_positions.get(&target_id))
            {
